    ...
    async def enter_bootloader(self) -> None:
        ...
        if self.app_type is ApplicationType.GECKO_BOOTLOADER:
            ...
            try:
                ...
                _LOGGER.warning(
                    f"CPC bootloader command failed: {e}. "
                    "Attempting RTS/DTR hardware reset as fallback..."
                )
                # CPC协议命令失败时，尝试使用RTS/DTR硬件复位
                # 这与Router固件使用相同的方法（参考Sonoff工具做法）
                try:
                    await self.trigger_bootloader(ResetTarget.RTS_DTR)
                    # 验证是否成功进入引导模式
                    await asyncio.sleep(0.5)  # 给设备时间进入引导模式
                    # 尝试探测引导模式以验证成功
                    try:
                        await self.probe_gecko_bootloader(
                            run_firmware=False,
                            baudrate=self._baudrates[ApplicationType.GECKO_BOOTLOADER][0]
                        )
                        _LOGGER.info("Successfully entered bootloader using RTS/DTR reset")
                    except asyncio.TimeoutError:
                        # 即使探测失败，也尝试继续（可能是设备还在初始化）
                        _LOGGER.warning(
                            "RTS/DTR reset performed, but bootloader probe timed out. "
                            "Continuing anyway..."
                        )
                except Exception as reset_error:
                    # 如果硬件复位也失败，抛出原始错误
                    raise RuntimeError(
                        f"Failed to enter bootloader via CPC command and RTS/DTR reset. "
                        f"CPC error: {e}, Reset error: {reset_error}"
                    ) from e
            ...
            try:
                ...
                _LOGGER.warning(
                    f"Router CLI bootloader command failed: {e}. "
                    "Attempting RTS/DTR hardware reset as fallback..."
                )
                # CLI命令失败时，尝试使用RTS/DTR硬件复位
                # 这是Sonoff等设备使用的可靠方法
                try:
                    await self.trigger_bootloader(ResetTarget.RTS_DTR)
                    # 验证是否成功进入引导模式
                    ...
                    # 如果硬件复位也失败，抛出原始错误
                    raise RuntimeError(
                        f"Failed to enter bootloader via CLI command and RTS/DTR reset. "
                        f"CLI error: {e}, Reset error: {reset_error}"
                    ) from e
