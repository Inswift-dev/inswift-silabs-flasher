...
class RouterProtocol(SerialProtocol):
    ...
    async def enter_bootloader(self) -> None:
        ...
        try:
            await self._state_machine.wait_for_state(State.READY, timeout=5.0)  # 增加超时时间
        except asyncio.TimeoutError:
            _LOGGER.warning("Router bootloader command timeout, assuming success")
            # 假设命令成功，继续执行
            self._state_machine.state = State.READY
