...
> **界面语言：** 程序窗口、按钮、对话框与 **Log** 区域中的运行日志均为**英文**。本文用中文说明操作含义；文中的 **粗体英文**与软件界面一致，便于对照。
...
- **Probe device**：只做类型识别，不会烧录（界面灰色说明含 *Probe order (Probe device only): …*）
- **Start flashing**：只做烧录，不会先做类型探测

探测顺序固定：`EZSP -> ROUTER -> RCP(SPINEL) -> CPC`
...
1. 点击 **Start flashing** 按钮
2. 程序直接进入引导模式（不做设备类型探测）
3. 观察日志输出
4. 等待烧录完成
...
**进度显示**:
- **进度条**（**Flash progress:** 区域）: 实时显示烧录进度，每1%更新一次
- **Log** 窗口: 每1%显示一次详细进度信息
  - 格式: `Flash progress: X% (X.XX MB / X.XX MB)`
  - 示例: `Flash progress: 45% (1.23 MB / 2.75 MB)`

### 7. 设备类型探测（可选）

如需仅识别设备类型，请单独点击 **Probe device** 按钮：

1. 连接设备并选择串口
2. 点击 **Probe device**
3. 查看日志中的探测结果（设备类型/版本/应用波特率）
...
**步骤**：
1. 在 **5. Write IEEE (EUI-64):** 区域输入 IEEE 地址
2. （可选）勾选 **Force overwrite**
3. 点击 **Write IEEE** 按钮
4. 确认操作

**常见错误**（界面与日志为英文，大意如下）：
- 日志中出现 `Error: device is not running EmberZNet; cannot write IEEE`：设备当前固件类型不支持，需要先烧录 EmberZNet 固件
- 弹窗 **Error** / `Invalid IEEE address`：检查地址格式，确保为 8 字节十六进制（见格式说明）

### Q8: 设备类型探测慢或失败

**说明**：
- 设备类型探测是独立功能，不参与烧录流程
- 探测波特率已固定：`SPINEL=460800`，其他协议均为 `115200`

**建议**：
1. 日常升级直接使用 **Start flashing**
2. 仅在需要确认设备状态时使用 **Probe device**
3. 若探测失败但可正常烧录，可直接烧录

### Q9: Router固件烧录后无法再次进入引导模式

**问题描述**：
- 设备在烧录 Router 固件后，再次烧录时日志中可能出现 `Failed to enter bootloader` / `Bootloader entry failed` 等英文提示
...
### Q10: Multi-PAN (CPC) 固件烧录后无法再次探测或进入引导模式

**问题描述**：
- 设备在烧录 Multi-PAN (CPC) 固件后，再次烧录时日志中可能出现 `Application probe failed` / `Failed to enter bootloader` 等英文提示
